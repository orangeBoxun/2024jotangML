...
def load_model(args):
    ...
    # is_seq2seq_model：判断是否为序列到序列模型
    args.is_seq2seq_model = any([(model_type in args.model_name_or_path) for model_type in ["t5","T0"]])
    # is_decoder_only_model：判断是否为仅解码模型
...
def main(args): 
    ...
    if not args.skip_model_load:
        ...
        term_width = 80             # 用于控制在控制台输出的文本宽度，使得输出更加整齐美观
        print("#"*term_width)       # 会在控制台打印出 80 个#字符组成的一行，用于分隔不同的输出部分，起到强调和区分的作用
        print("Prompt:")
        print(input_text)
